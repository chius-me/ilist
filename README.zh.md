...
[![Release](https://img.shields.io/badge/release-v0.2.1-2ea44f?logo=github)](https://github.com/chius-me/ilist/blob/main/docs/releases/v0.2.1.md)
...
> [v0.2.1](docs/releases/v0.2.1.md) 增加按挂载保存的 OAuth 凭据、可选的 OneDrive 根文件夹、iList 产品名，以及资源管理器界面修复。升级前请阅读[发布指南](docs/releases/v0.2.1.md)。v0.1.8 安全加固仍然必需，见 [v0.1.8](docs/releases/v0.1.8.md)。
...
5. **生成管理员密码哈希和随机密钥。**
   ...
   ```bash
   npm run hash-password   # ADMIN_PASSWORD_HASH；按提示输入密码
...
| `npm run hash-password` | 交互输入并生成管理员密码哈希 |
...
- 同源 HTML、SVG、XML、PDF 和未知类型文件会作为附件下载，并使用隔离的文件响应策略。只有精确的少数图片、音频和视频 MIME 类型可内联预览。
...
`v0.1.x` 版本旨在继续兼容旧版 R2 对象链接。迁移会添加条目和挂载模型，但不会删除文件或旧版行。v0.1.7 安全版本请遵循[发布中止流程](docs/releases/v0.1.7.md#emergency-abort-and-rollback)：不要将公开流量回退到 v0.1.6，并保留 `0015` 和 `0016`。只有数据本身损坏或迁移失败时才恢复 D1 导出。
