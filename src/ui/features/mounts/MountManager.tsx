import { ArrowLeft, CirclePower, Cloud, Pencil, Plus, RefreshCw, Trash2 } from 'lucide-react';
import { useCallback, useEffect, useState } from 'react';
import { createMount, listMounts, removeMount, testMount, updateMount } from '../../api/mounts';
import type { Mount, MountInput } from '../../types/mounts';
import { MountDialog } from './MountDialog';

export function MountManager({ onBack }: { onBack: () => void }) {
  ...
  async function save(input: MountInput) {
    ...
    try {
      if (editing) await updateMount(editing.id, input); else await createMount(input);
  ...
  return <main className="storageManager" id="storage-manager">
    ...
    <div className="mountList">{mounts.map((mount) => <article className="mountRow" key={mount.id}>
      ...
      <div className="mountDetails"><span>{mount.provider}</span><span>{String(mount.config.bucket ?? '')}</span><span className={mount.enabled ? 'statusOnline' : 'statusOffline'}>{mount.enabled ? 'Enabled' : 'Disabled'}</span></div>
