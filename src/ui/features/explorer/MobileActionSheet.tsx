...
export function MobileActionSheet({ open, title, actions, onClose }: {
  open: boolean;
  title: string;
      ...
      <section className="mobileActionSheet" role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="mobileActionList">
          {actions.map((action, index) => {
            ...
            const content = <>{Icon ? <Icon aria-hidden={true} size={18} /> : null}{action.label}</>;
        ...
        <button className="mobileActionCancel" type="button" onClick={onClose}>Cancel</button>
      </section>
    </div>
  );
}
