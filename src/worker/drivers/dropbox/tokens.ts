...
export interface DropboxCredentials extends Record<string, unknown> {
  accessToken: string;
  refreshToken: string;
  tokenType: string;
  expiresAt: number;
...
function validCredentials(value: Record<string, unknown> | null): DropboxCredentials {
  ...
  ) throw new HttpError(409, 'DROPBOX_NOT_CONNECTED', 'Dropbox mount is not connected');
  return {
...
async function waitForRefresh(env: Env, mountId: string): Promise<string> {
  ...
  throw new HttpError(503, 'DROPBOX_REFRESH_BUSY', 'Dropbox token refresh is busy');
}
...
export async function getDropboxAccessToken(env: Env, mountId: string): Promise<string> {
  const credentials = validCredentials(await getCredentials(env, mountId));
  if (credentials.expiresAt > Date.now() + REFRESH_SKEW_MS) return credentials.accessToken;
